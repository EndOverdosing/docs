...
Tokens are HMAC-signed and verified server-side without shared in-memory state, so they work correctly across all server workers. Tokens expire after 30 minutes. Before they expire, refresh them with `POST /api/auth/refresh` instead of re-issuing from `/api/auth` — this avoids sending your API key to the browser again.

### Step 3 — Refresh the token (client-side)

```javascript
const res = await fetch('https://api.vyla.cc/api/auth/refresh', {
  method: 'POST',
  headers: { 'X-Session-Token': token }
});
...
Tokens can be refreshed up to 10 minutes after they expire. Refresh preserves the tier of the original token. If the API key used to issue the token has since been disabled or changed tier, refresh will fail and you must re-authenticate via `/api/auth`.
...
| `Session token cannot be refreshed` | Token is malformed, more than 10 minutes past expiry, or its source API key was disabled/changed. Re-fetch from `/api/auth`. |
...
API keys are no longer issued. To access Vyla’s streaming providers and features, use the official SDK instead.

Read the SDK documentation: https://docs.vyla.cc/sdk
...
| `POST /api/auth/refresh` | Yes (session token) | Yes |
| `GET /api?url=` | Yes | No |